		...
		try {	
			var exito = function(response) {
				try {
					...
					alert("Error", "Se produjo un error al momento de consultar los departamentos",	e);
				}
			}
			var error = function(response) {
			...
			smartServices.sendPost(angular.toJson(sendObject),
					hostSmart + context	+ methodListarDepartamento, exito, error);
	
		} catch (error) {
			alert("Error", "Se produjo un error al momento de listar los departamentos", error.message);
		}
	}
		...
		try {
			if ($scope.departamento != null) {
				...
				smartServices.sendPost(angular.toJson(sendObject), hostSmart
						+ context + methodListarCiudad, exito,error);
			}
	...
	$scope.grabarEmpresa = function() {
		try {
			...
			if ($scope.sw != false) {
				...
				smartServices.sendPost(angular.toJson(sendObjectEmpresa), hostSmart
						+ context + methodGrabarEmpresa, exito, error);
			}
		} catch (error) {
